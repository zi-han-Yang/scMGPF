...
class SCOT(object):
    ...
    def normalize(self, norm="l2", bySample=True):
        assert (norm in ["l1", "l2", "max",
                         "zscore"]), "Norm argument has to be either one of 'max', 'l1', 'l2' or 'zscore'. If you would like to perform another type of normalization, please give SCOT the normalize data and set the argument normalize=False when running the algorithm."
    ...
    def entropic_gromov_wasserstein(self, iy, loss_fun, epsilon, rho=0.05, max_iter=1000, tol=1e-9, verbose=False,
                                    balanced=True):
        """
        ...
        while (err > tol and cpt < max_iter):
            ...
            if balanced:
                ...
                T = ot.unbalanced.sinkhorn_unbalanced(self.p, self.q[iy], tens, epsilon,
                                                      rho, method='sinkhorn', numItermax=max_iter, stopThr=tol,
