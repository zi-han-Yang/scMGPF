...
class JointMDS:
    ...
    def fit_transform(self, D1, D2, w1=None, w2=None, a=None, b=None):
        ...
        if self.gw_init:
            self.coupling_ = gromov_wasserstein(
            ...
            # from visnew2 import visualize2
            # import numpy as np

            # F = np.eye(2000)
            # visualize2([Z_old[:m], Z_old[m:]], [Z_old[:m], Z_old[m:]], F, [y1, y2], mode='DIR', method="AE")
        ...
        for i in pbar:
            ...
            # import numpy as np
            # F = np.eye(2000)
            # visualize2([Z[:m], Z[m:]], [Z[:m], Z[m:]], F, [y1, y2], mode='DIR', method="AE")
