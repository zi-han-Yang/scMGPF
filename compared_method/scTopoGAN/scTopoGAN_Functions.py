...
def prepare_manifold_data(Manifold_Data, batch_size):
    ...
    all_samples = list(Manifold_Data.obs_names)
    Manifold_Data = np.array(Manifold_Data.X)
    ...
    for epoch in range(n_epochs):
        ...
        for train_batch_id, data in enumerate(dataloaders['train']):
            ...
            torch.nn.utils.clip_grad_norm_(model.parameters(),
                                           grad_threshold)  # Normalise gradients to prevent gradient explosion
            optimizer.step()
...
def get_TopoAE_Embeddings(Manifold_Data, batch_size, autoencoder_model, AE_arch, topology_regulariser_coefficient,
                          initial_LR):
...
def run_scTopoGAN(source_tech, target_tech, source_tech_name, target_tech_name, batch_size,
    ...
    for random_seed in range(num_iterations):
        ...
        generator = generator.to(device)  # "cuda:0"
        ...
        for epoch in epochs:
            ...
            model = torch.load(path, weights_only=False).to('cpu')  # Load trained model
            model.eval()
    ...
    model_01_name = "{}_to_{}_Generator_{}_{}_MODEL_{}".format(source_tech_name, target_tech_name, epoch_01,
                                                               core_suffix, model_01)

    path_01 = "{}/Models/{}.pt".format(path_prefix, model_01_name)

    # Load model
    generator_1 = torch.load(path_01, map_location=device)  # "cuda:0"
    ...
    source_tech = source_tech.clone().detach().float()
    data_loader = DataLoader(source_tech, batch_size=1)
    for index, original_sample in enumerate(data_loader):
