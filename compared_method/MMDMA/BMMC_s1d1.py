...
def main():
    ...
    for seed in range(1):
        ...
        for i in tqdm(range(10001), desc="Training Progress"):  # Training takes place for 10,000 iterations
    ...
    x1 = np.matmul(k1_matrix, alpha)
    x2 = np.matmul(k2_matrix, beta)
    inte = [x1.cpu().detach().numpy(), x2.cpu().detach().numpy()]
    MMDMA_inte = dict({"inte": inte})
...
# MMDMA_inte = np.load('../results/BMMC_s1d2/MMDMA/MMDMA.npy',allow_pickle=True).item()
# inte = [MMDMA_inte['inte'][0].cpu().detach().numpy(),MMDMA_inte['inte'][1].cpu().detach().numpy()]
# MMDMA_inte = dict({"inte": inte})
# np.save('../results/BMMC_s1d2/MMDMA/MMDMA.npy', MMDMA_inte)
