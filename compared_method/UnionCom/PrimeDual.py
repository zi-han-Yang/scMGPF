...
def cor_pairs_match(Kx, Ky, N, params, p1, p2, device):
    ...
    while (i < params.epoch_pd):
        grad = 4 * torch.mm(F, torch.mm(Ky, torch.mm(torch.t(F), torch.mm(F, Ky)))) \
               ...
               + torch.mm(Im, torch.t(Lambda)) + params.rho * (
                       torch.mm(F, torch.mm(In, torch.t(In))) - torch.mm(Im, torch.t(In)) + torch.mm(Im, torch.mm(
                   torch.t(Im), F)) + torch.mm(Im, torch.t(S - In)))
        i += 1
