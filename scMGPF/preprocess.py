...
def preprocess_rna(adata_rna, target_sum=1e4, n_top_genes=2000, n_components=50, device='cuda'):
    if adata_rna is None or adata_rna.n_obs == 0 or adata_rna.n_vars == 0:
        raise ValueError("RNA-seq data is empty and cannot be processed")
    ...
    if not np.all(np.modf(x_array)[0] == 0):
        print(" Non-integer value detected. Data may have been standardized ")
...
def preprocess_atac(adata_atac, n_components=50, use_highly_variable=None, device='cuda'):
    if adata_atac is None or adata_atac.n_obs == 0 or adata_atac.n_vars == 0:
        raise ValueError("ATAC-seq data is empty and cannot be processed ")
    ...
    X = scipy.sparse.csr_matrix(adata_use.X)  # Ensure sparse format
    ...
    tfidf = normalize(tfidf, norm="l1")  # Can try l2 or use normalize_data in preprocess.py
...
def normalize_data(data, norm="l2", bySample=True):  # l2l2 normalization
